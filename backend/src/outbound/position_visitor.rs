    ...
    fn san(
        ...
        match san.to_move(&pos) {
            Ok(mv) => {
                ...
                match next_fen {
                    Ok(fen) => movetext.result.push(fen.into()),
                    Err(_) => return ControlFlow::Break(Err(InvalidPgnError(self.pgn.clone()))),
                };
                ControlFlow::Continue(())
            }
...
mod tests {
    ...
    fn test_pgn_success() {
        ...
        let expected_fens: Result<Vec<Fen>, InvalidPgnError> = Ok(vec![
            Fen::new_unchecked("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1"),
            Fen::new_unchecked("rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w KQkq e6 0 2"),
            Fen::new_unchecked("rnbqkbnr/pppp1ppp/8/4p3/2B1P3/8/PPPP1PPP/RNBQK1NR b KQkq - 1 2"),
            Fen::new_unchecked("r1bqkbnr/pppp1ppp/2n5/4p3/2B1P3/8/PPPP1PPP/RNBQK1NR w KQkq - 2 3"),
            Fen::new_unchecked("r1bqkbnr/pppp1ppp/2n5/4p2Q/2B1P3/8/PPPP1PPP/RNB1K1NR b KQkq - 3 3"),
            Fen::new_unchecked(
        ...
        assert_eq!(actual_fens, expected_fens);
    }
